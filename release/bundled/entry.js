 ...
 * @LastEditors: 谢树宏
 * @LastEditTime: 2022-01-25 17:03:56
...
function getExpireTime(intervalTime) {
    ...
    return `${expireTime.getFullYear()}-${expireTime.getMonth() + 1}-${expireTime.getDate()} ${expireTime.getHours()}:${expireTime.getMinutes()}`;
}
/**
 * 获取所有文件
...
function findAllFile(dirPath, ignoreDir = ['node_modules', '.git']) {
    ...
    files.forEach((file) => {
        if (!ignoreDir.includes(file)) {
            ...
            if (stat.isDirectory()) {
                filtedFiles.push(...findAllFile(path__default["default"].join(dirPath, file), ignoreDir));
            }
            else {
...
async function preview(event, params) {
    ...
    const qrcodeOutputDest = path__default["default"].resolve(__dirname, `../../image/${uuid$1}.png`);
    console.log('触发预览');
    try {
