  ...
  files.forEach((file) => {
    if (!ignoreDir.includes(file)) {
      ...
      if (deep) {
        if (stat.isDirectory()) {
          filtedFiles.push(
            ...findAllFile(path__default['default'].join(dirPath, file), true, ignoreDir)
          )
        } else {
      ...
      onProgressUpdate: (res) => {
        if (res._msg !== 'upload') {
          ...
          !showIndex &&
            event.reply(
              'uploadReply',
              new Reply(SUCCESS, { message: `正在上传中`, index: params.index, done: false })
            )
          showIndex++
...
async function preview(event, params) {
  ...
  if (!existFile(electron.app.getPath('documents'), 'mp-image', false)) {
    fs__default['default'].mkdirSync(
      path__default['default'].join(electron.app.getPath('documents'), 'mp-image')
    )
  }
  const qrcodeOutputDest = path__default['default'].join(
    ...
    const previewResult = await ci__default['default'].preview({
      ...
      onProgressUpdate: (res) => {
        ...
        if (res._msg !== 'upload') {
          event.reply(
            'previewReply',
            new Reply(SUCCESS, { message: `正在编译中`, index: params.index, done: false })
          )
        } else if (res._msg === 'upload' && res._status === 'done') {
    ...
    case 'reset':
      ...
      try {
        for (const item of Array.isArray(params?.list) ? params?.list : [params?.list]) {
          await git__default['default'].resetIndex({
            fs: fs__default['default'],
      ...
      try {
        for (const item of Array.isArray(params?.list) ? params?.list : [params?.list]) {
          if (item.status.includes('delete')) {
            await git__default['default'].remove({
              fs: fs__default['default'],
              dir: params.project.path,
              filepath: item.path
            })
          } else {
            await git__default['default'].add({
              fs: fs__default['default'],
              dir: params.project.path,
              filepath: item.path
            })
  ...
  switch (arg.type) {
    ...
    case 'export':
      if (filePaths.length) {
        ...
        if (existGitDir) {
          // 获取当前项目下的所有分支
          branches = await git__default['default'].listBranches({
            fs: fs__default['default'],
            dir: selectPath
          })
          // 获取当前项目下的当前分支
        ...
        if (existFile(selectPath, 'pages.json')) {
          ...
          filterObject = []
          const existManifest = fs__default['default'].existsSync(
            path__default['default'].join(selectPath, 'manifest.json')
          )
          ...
          includesArray.forEach((item, index) => {
            filterObject.push({
              ...data,
              outputPath: path__default['default'].join(
                selectPath,
          ...
          fs__default['default'].existsSync(path__default['default'].join(selectPath, 'app.json'))
        ) {
        ...
        event.reply(
          'selectFolderReply',
          new Reply(SUCCESS, { message: '添加成功', data: filterObject })
        )
      }
      break
    // 导入项目配置路径
...
async function excuteCommon(event, arg) {
  ...
  switch (type) {
    ...
    case 'refresh':
      ...
      if (existGitDir) {
        // 获取当前项目下的所有分支
        const branches = await git__default['default'].listBranches({
          fs: fs__default['default'],
          dir: params.path
        })
        // 获取当前项目下的当前分支
 ...
 * @LastEditors: 谢树宏
 * @LastEditTime: 2022-01-26 11:05:03
